...
const PN_MESSAGE = {
  ...
  android: {
    ...
    restrictedPackageName: 'sg.gov.homer', // This ensures message is only sent to this app
...
const SMS_MESSAGE = '[Ministry of Health] Please download and launch the Homer App now for us to automatically confirm your current location.'
...
async function main ({ firebaseCredentialFilePath, twilioId, twilioToken, twilioAlphaSender, twilioMessagingServiceId, pushNotificationFilePath, smsFilePath }) {
  ...
  await bluebird.mapSeries(
    smses,
    sms => {
      console.log(`sending sms for ${sms.qo}`)
      return twilio.messages.create({
        body: SMS_MESSAGE,
        messagingServiceSid: twilioMessagingServiceId,
        from: twilioAlphaSender,
        to: `+${sms.phone}`
      })
      .catch(e => console.log(`error while sending : ${e.name}`))
    }
...
program.requiredOption('--twilioAlphaSender <twilioAlphaSender>', 'Twilio Alpha Sender')
program.requiredOption('--twilioMessagingServiceId <twilioMessagingServiceId>', 'Twilio Messaging Service Id')
